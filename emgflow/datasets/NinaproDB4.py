...
from emgflow.datasets.utils.split import SplitSpec, split_index
from emgflow.datasets.utils.transforms import minmax_norm, wavelet_denoise_db7


class NinaproDB4Simple(Dataset):
    """
    Sliding window dataset for DB4 `.npy` files.

    DB4 now shares the same top-level storage format as DB2/DB7 in this
    workspace: each file stores `emg`, `label`, `repetition`, and `meta`.
    Labels are assumed to be pre-mapped during preprocessing, so no runtime
        ...
        if self.norm_method == "zscore":
            if self.mean is None and self.std is None:
                if split_spec is not None:
                    ...
                    raise ValueError(
                        "For zscore normalization, 'split_spec' must be provided "
                        "to compute statistics on training set (or provide 'mean' and 'std' manually)."
                    )
    ...
    def _build_index(self):
        for subj in self.subjects:
            ...
            for npy_file in sorted(subj_dir.glob("*.npy")):
                ...
                meta = data.get("meta", {})
                trials = meta.get("total_trials") if isinstance(meta, dict) else None
                if trials is None:
                    trials = len(labels)
                ...
                for trial_id in range(trials):
                    ...
                    for start in range(0, t_total - win + 1, stride):
                        end = start + win
                        self.samples.append(
    ...
    def _load_file(self, npy_path):
        ...
        if self.use_disk_cache:
            try:
                ...
                print(f"Failed to save DB4 cache {cache_path}: {e}")
    ...
    def subset(self, samples, **kwargs):
        ...
        return NinaproDB4Simple(**params)
    ...
    def compute_mean_std(self, spec: SplitSpec):
        ...
        for subj in subjects_in_train:
            ...
            for x_batch, _ in loader:
                ...
                if sum_x is None:
                    sum_x = torch.zeros(channels, device=device)
                    sum_sq_x = torch.zeros(channels, device=device)
