...
class NinaproDB7Simple(Dataset):
    """
    Sliding window dataset for DB7 `.npy` files.

    DB7 shares the same top-level storage format as DB2 in this workspace:
    each file stores `emg`, `label`, `repetition`, and `meta`.
    """
    ...
    def _load_file(self, npy_path):
        ...
        if self.use_disk_cache:
            try:
                ...
                print(f"Failed to save DB7 cache {cache_path}: {e}")
    ...
    def subset(self, samples, **kwargs):
        ...
        return NinaproDB7Simple(**params)
